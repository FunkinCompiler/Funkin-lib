...
{
  public static final APP_ID:Null<String> = EnvironmentConfigMacro.environmentConfig?.get("API_NG_APP_ID");
  public static final ENCRYPTION_KEY:Null<String> = EnvironmentConfigMacro.environmentConfig?.get("API_NG_ENC_KEY");

  public static var instance(get, never):NewgroundsClient;

  public static var _instance:Null<NewgroundsClient> = null;

  public static function get_instance():NewgroundsClient
  {
  ...
  public function onLoginResolved(outcome:LoginOutcome):Void
  {
    switch (outcome)
    {
      case SUCCESS:
  ...
  public function onLoginResolvedWithCallbacks(outcome:LoginOutcome, onSuccess:Void->Void, onError:Void->Void):Void
  {
  ...
  public function onLogoutResolved(outcome:Outcome<CallError>):Void
  {
    switch (outcome)
    {
      case SUCCESS:
  ...
  public function onLogoutResolvedWithCallbacks(outcome:Outcome<CallError>, onSuccess:Void->Void, onError:Void->Void):Void
  {
  ...
  public function onLoginSuccessful():Void
  {
    if (NG.core == null) return;
  ...
  public function onLoginFailed(result:LoginFail):Void
  {
    switch (result)
    {
      case CANCELLED(type):
  ...
  public function onLogoutSuccessful():Void
  {
    trace('[NEWGROUNDS] Logout successful!');
  }

  public function onLogoutFailed(result:CallError):Void
  {
    switch (result)
    {
      case HTTP(error):
  ...
  public function onFetchedMedals(outcome:Outcome<CallError>):Void
  {
    trace('[NEWGROUNDS] Fetched medals!');
  }

  public function onFetchedLeaderboards(outcome:Outcome<CallError>):Void
  {
  ...
  public function onFetchedSaveSlots(outcome:Outcome<CallError>):Void
  {
  ...
  public function get_user():Null<User>
  {
    if (NG.core == null || !this.isLoggedIn()) return null;
    return NG.core.user;
  }

  public function get_medals():Null<MedalList>
  {
    if (NG.core == null || !this.isLoggedIn()) return null;
    return NG.core.medals;
  }

  public function get_leaderboards():Null<ScoreBoardList>
  {
    if (NG.core == null || !this.isLoggedIn()) return null;
    return NG.core.scoreBoards;
  }

  public function get_saveSlots():Null<SaveSlotList>
  {
    if (NG.core == null || !this.isLoggedIn()) return null;
    return NG.core.saveSlots;
  }

  public static function getSessionId():Null<String>
  {
    #if js
